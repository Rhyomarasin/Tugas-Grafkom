...
void Demo::Render() {
	...
	glm::mat4 view = glm::lookAt(glm::vec3(0, 2, 6), glm::vec3(0, 0, 0), glm::vec3(0.0f, 1.0f, 0.0f));
	GLint viewLoc = glGetUniformLocation(this->shaderProgram, "view");
	glUniformMatrix4fv(viewLoc, 1, GL_FALSE, glm::value_ptr(view));

	DrawColoredRefrigerator();
	DrawColoredWall();
	DrawColoredPlane();
	DrawColoredPaint();
...
void Demo::BuildColoredRefrigerator() {
	...
	int width, height;
	unsigned char* image = SOIL_load_image("Refrigerator.png", &width, &height, 0, SOIL_LOAD_RGBA);
...
void Demo::DrawColoredRefrigerator()
{
...
void Demo::BuildColoredPaint() {
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	int width, height;
	unsigned char* image = SOIL_load_image("Paint.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
